"""
Pour la creation d'un workspace vierge puis tansfert du workspace vers le owner de la wallet
...
# main function called by Web_create_identity.py
def create_user(wallet_address, username, email, mode, user_aes_encrypted_with_talao_key = None, rsa=None, secret=None, private=None, password=None, firstname=None,  lastname=None, phone=None, decentralized=True):

	if decentralized :
		print('Warning : decentralized mode')
	else :
		print('Warning : centralized mode')

	# STEP 1 : create a worskpace contract with a random ethereum address
	address, private_key, workspace_contract = _create_user_step_1(wallet_address, email, mode, firstname,  lastname, rsa, private, secret, decentralized)
	if not address :
		return False

	# STEP 2 : Add keys to Identity
	_create_user_step_2(wallet_address, address, workspace_contract, private_key, username, email, mode, user_aes_encrypted_with_talao_key, decentralized)

	# STEP 3 : transfer ownership of workspace contract to wallet address or setup an alias and send emails
	if not _create_user_step_3(address, private_key, wallet_address, workspace_contract, username, email, password, phone, mode, decentralized) :
		return False

	return True

def _create_user_step_1(wallet_address, email,mode, firstname, lastname, rsa, private, secret, decentralized) :
...
def _create_user_step_2(wallet_address, address, workspace_contract, private_key, username, email, mode, user_aes_encrypted_with_talao_key, decentralized) :
	...
	if partnershiprequest(mode.owner_talao, mode.workspace_contract_talao, mode.owner_talao, mode.workspace_contract_talao, talao_private_key, workspace_contract, talao_rsa_key, mode, user_aes_encrypted_with_talao_key) :
		...
		if decentralized :
			...
			if receipt['status']  :
				print('Success : Talao partnership request has been accepted by Identity (decentralized)')
			else :
				print('Error : authorize partnership with Talao failed (decentralized)')
		else :
			RSA_private = privatekey.get_key(address, 'rsa_key', mode)
			if authorize_partnership(address, workspace_contract, address, workspace_contract, private_key, mode.workspace_contract_talao, RSA_private, mode) :
				print('Success : Talao partnership request has been accepted by Identity (centralized)')
			else :
				print('Error : authorize partnership with Talao failed (centralized)')
...
def _create_user_step_3(address, private_key, wallet_address, workspace_contract, username, email, password, phone, mode, decentralized) :
